...
BEGIN
     ...
     SET @NewLogin = 'User' + CAST(NEWID() AS NVARCHAR(36));
        IF NOT EXISTS (SELECT 1 FROM ПользователиЛичное WHERE Логин = @NewLogin)
            SET @UniqueLogin = 1;
     END;
    ...
    UPDATE ПользователиЛичное
    SET  Логин = @NewLogin
    WHERE ID_ПользователяЛичное = (SELECT ID_ПользователяЛичное FROM inserted);
